...
func main() {
	...
	graph := compose.NewGraph[map[string]string, *schema.Message](
		compose.WithGenLocalState(genFunc), //	加上全局State
	)
	...
	lambda := compose.InvokableLambda(func(ctx context.Context,
		...
		_ = compose.ProcessState[*State](ctx, func(ctx context.Context, state *State) error {
			state.History["cute_action"] = "捏捏脸"
			state.History["tsundere_action"] = "生生气"
			return nil
		})
	...
	//	使用 State
	cutePreHandler := func(ctx context.Context,
		input map[string]string, state *State) (output map[string]string, err error) {
		input["content"] = input["content"] + state.History["cute_action"].(string)
		return input, nil
	}
	tsunderePreHandler := func(ctx context.Context,
		input map[string]string, state *State) (output map[string]string, err error) {
	...
	_ = graph.AddLambdaNode("cutelambda", cutelambda, compose.WithStatePreHandler(cutePreHandler))
	_ = graph.AddLambdaNode("tsunderelambda", tsunderelambda, compose.WithStatePreHandler(tsunderePreHandler))
